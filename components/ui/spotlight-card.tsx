...
const glowStyles = `
  ...
  [data-glow-card] > [data-glow-card-inner]::before {
    ...
    background-image: radial-gradient(
      ...
      calc(var(--x, 0) * 1px)
      calc(var(--y, 0) * 1px),
      hsl(var(--hue, 210) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 50) * 1%) / 0.45),
      transparent 100%
    );
    mask: linear-gradient(transparent, transparent), linear-gradient(white, white);
    mask-clip: padding-box, border-box;
...
export function GlowCard({
  ...
  const cardStyle: GlowStyle = {
    ...
    "--radius": 8,
    "--border": 2,
    "--backdrop": "hsl(240 10% 4% / 0.18)",
    "--backup-border": "hsl(0 0% 100% / 0.28)",
    "--size": 230,
    "--outer": 1,
    "--saturation": 95,
    "--lightness": 70,
    "--bg-spot-opacity": 0.13,
  ...
  return (
    <>
      ...
      <div
        ...
        data-glow-card
        style={cardStyle}
        className={`relative overflow-hidden rounded-lg shadow-[0_1rem_2rem_-1rem_black] backdrop-blur-[5px] ${
